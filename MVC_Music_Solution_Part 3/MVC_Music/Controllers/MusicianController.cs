...
namespace MVC_Music.Controllers
{
    ...
    public class MusicianController : ElephantController
    {
        ...
        public async Task<IActionResult> PerformancesSummary(int? page, int? pageSizeID)
        {
            var sumQ = GePerformanceSummaries();

            int pageSize = PageSizeHelper.SetPageSize(HttpContext, pageSizeID, "PerformancesSummary");//Remember for this View
            ViewData["pageSizeID"] = PageSizeHelper.PageSizeList(pageSize);
            var pagedData = await PaginatedList<PerformancesSummaryVM>.CreateAsync(sumQ.AsNoTracking(), page ?? 1, pageSize);
        ...
        public IActionResult AappointmentSummaryExport()
        {
            ...
            int endRow = 4 + numRows;
            int summaryStartRow = endRow + 2;
            if (numRows > 0) //We have data
            {
                ...
                using (ExcelPackage excel = new ExcelPackage())
                {
                    ...
                    workSheet.Cells[summaryStartRow, 8].Value = "Total Performances:";
                    using (ExcelRange totalPerformancesRange = workSheet.Cells[summaryStartRow, 9])
                    {
                        string startCell = workSheet.Cells[4, 9].Address;
                        string endCell = workSheet.Cells[endRow, 9].Address;

                        totalPerformancesRange.Formula = $"SUM({startCell}:{endCell})";
                        totalPerformancesRange.Style.Font.Bold = true;
                        totalPerformancesRange.Style.Numberformat.Format = "#,##0";
                    }

                    workSheet.Cells[summaryStartRow + 1, 1].Value = "Total Musicians in Report:";
                    workSheet.Cells[summaryStartRow + 1, 2].Value = numRows;
                    workSheet.Cells[summaryStartRow + 1, 2].Style.Font.Bold = true;
        ...
        private IQueryable<PerformancesSummaryVM> GePerformanceSummaries()
        {
            return _context.Performances.Include(a => a.Musician)
                .GroupBy(a => new { a.MusicianID, a.Musician!.LastName, a.Musician!.FirstName, a.Musician!.MiddleName })
                .Select(grp => new PerformancesSummaryVM
                {
                    Id = grp.Key.MusicianID,
                    First_Name = grp.Key.FirstName,
                    Middle_Name = grp.Key.MiddleName,
                    Last_Name = grp.Key.LastName,
                    Total_Of_Performances = grp.Count(),
                    Average_Fee = grp.Average(a => a.FeePaid),
                    Highest_Fee = grp.Max(a => a.FeePaid),
                    Fewest_Fee = grp.Min(a => a.FeePaid)
                })
