...
namespace MVC_Music.Controllers
{
    ...
    public class InstrumentController : ElephantController
    {
        ...
        //    List<Instrument> instrumentsAdd = new List<Instrument>();
        ...
        //        };
        //        instrumentsAdd.Add(a);
        //    }
        //    _context.Instruments.AddRange(instrumentsAdd);
        ...
        public async Task<IActionResult> InsertFromExcel(IFormFile theExcel)
        {
            ...
            if (theExcel != null)
            {
                ...
                {
                    if (mimeType.Contains("excel") || mimeType.Contains("spreadsheet") || mimeType.Contains("openxmlformats-officedocument"))
                    {
                        ExcelPackage excel;
                        using (var memoryStream = new MemoryStream())
                        {
                        ...
                        if (workSheet.Cells[1, 1].Text == "Instrument Name")
                        {
                            for (int row = start.Row + 1; row <= end.Row; row++)
                            {
                                ...
                                try
                                {
                                    currentInstrument.Name = workSheet.Cells[row, 1].Text.Trim();

                                    if (string.IsNullOrWhiteSpace(currentInstrument.Name))
                                    {
                                        continue;
                                    }

                                    _context.Instruments.Add(currentInstrument);
                                    _context.SaveChanges();
                                    successCount++;
                                }
                                ...
                                {
                                    errorCount++;
                                    if (dex.GetBaseException().Message.Contains("UNIQUE constraint failed") || dex.GetBaseException().Message.Contains("duplicate key"))
                                    {
                                        feedBack += $"Error: Instrument '{currentInstrument.Name}' was rejected as a duplicate." + "<br />";
                                    }
                                    else
                                    {
                                        feedBack += $"Error: Instrument '{currentInstrument.Name}' caused a database error." + "<br />";
                                    }
                                    _context.Remove(currentInstrument);
                                }
                                catch (Exception ex)
                                {
                                    errorCount++;
                                    feedBack += $"Error: Instrument '{currentInstrument.Name}' caused an unexpected error. Message: {ex.Message}" + "<br />";
                                }
                            }
